"""Phase 3 gate: ``LLM.generate()`` on the tiny Qwen fixture via Spike."""
...
from nanovllm.backends.spike.engine_golden import ensure_tiny_fixture, tiny_llm_kwargs
...
def tiny_fixture() -> str:
    return ensure_tiny_fixture(seed=0)


@pytest.fixture(scope="module")
def prompt_tokens(tiny_fixture: str) -> list[int]:
    meta_path = os.path.join(tiny_fixture, "fixture_meta.json")
...
def _generate(
    ...
    **report_kwargs,
    ...
    build_dir = Path(f"demos/build_rvv/tiny_qwen_{build_suffix}")
    ...
    try:
        ...
        outputs = llm.generate(
            [prompt_tokens],
            SamplingParams(temperature=1e-9, max_tokens=2),
            use_tqdm=False,
        )
...
@pytest.mark.slow
def test_spike_generate_tiny_parity(tiny_fixture: str, prompt_tokens: list[int]):
    ref_build = Path("demos/build_rvv/tiny_qwen_cpu_ref")
    ref_ids, _ = _generate(
        tiny_fixture,
        prompt_tokens,
        spike=False,
